        ...
        sn_lookup{std::move(lookup)}, log_lvl{level}, logger{std::move(logger)}
{
    ...
    LMQ_TRACE("Constructing SispopMQ, id=", object_id, ", this=", this);
    ...
    if (pubkey.empty() != privkey.empty()) {
        ...
        LMQ_LOG(debug, "generating x25519 keypair for remote-only SispopMQ instance");
...
void SispopMQ::start() {
    ...
    LMQ_LOG(info, "Initializing SispopMQ ", bind.empty() ? "remote-only" : "listener", " with pubkey ", to_hex(pubkey));
    ...
    if (MAX_SOCKETS > 1 && MAX_SOCKETS <= zmq_socket_limit)
        ...
        LMQ_LOG(error, "Not applying SispopMQ::MAX_SOCKETS setting: ", MAX_SOCKETS, " must be in [1, ", zmq_socket_limit, "]");
    ...
    proxy_thread = std::thread{&SispopMQ::proxy_loop, this};
...
void SispopMQ::listen_curve(std::string bind_addr, AllowFunc allow_connection) {
...
void SispopMQ::listen_plain(std::string bind_addr, AllowFunc allow_connection) {
    // TODO: As above.
    check_not_started(proxy_thread, "start listening");
...
SispopMQ::~SispopMQ() {
    ...
    LMQ_LOG(info, "SispopMQ shutting down proxy thread");
    detail::send_control(get_control_socket(), "QUIT");
    proxy_thread.join();
    LMQ_LOG(info, "SispopMQ proxy thread has stopped");
}
