...
void SispopMQ::proxy_send(bt_dict_consumer data) {
...
void SispopMQ::proxy_reply(bt_dict_consumer data) {
    bool have_conn_id = false;
...
void SispopMQ::proxy_control_message(std::vector<zmq::message_t>& parts) {
...
void SispopMQ::proxy_loop() {
    ...
    for (size_t i = 0; i < bind.size(); i++) {
        ...
        LMQ_LOG(info, "SispopMQ listening on ", bind[i].first);
...
// reason)
bool SispopMQ::proxy_handle_builtin(size_t conn_index, std::vector<zmq::message_t>& parts) {
    ...
    if (cmd == "REPLY") {
        ...
        if (parts.size() == (1 + incoming) && cmd == "UNKNOWNCOMMAND") {
            ...
            LMQ_LOG(debug, "Received plain UNKNOWNCOMMAND; remote is probably an older sispopmq. Ignoring.");
            return true;
        }
